...
const CodeEditorJavascript = lazy(() => import("../examples/code-editor/javascript"));
const CodeEditorTypescript = lazy(() => import("../examples/code-editor/typescript"));
const CodeEditorJson = lazy(() => import("../examples/code-editor/json"));
const CodeEditorHtml = lazy(() => import("../examples/code-editor/html"));
const CodeEditorCss = lazy(() => import("../examples/code-editor/css"));
const CodeEditorNoLineNumbers = lazy(() => import("../examples/code-editor/no-line-numbers"));

import codeEditorCssSource from "../examples/code-editor/css?raw";
import codeEditorDefaultSource from "../examples/code-editor/default?raw";
import codeEditorHtmlSource from "../examples/code-editor/html?raw";
import codeEditorJavascriptSource from "../examples/code-editor/javascript?raw";
import codeEditorJsonSource from "../examples/code-editor/json?raw";
import codeEditorNoLineNumbersSource from "../examples/code-editor/no-line-numbers?raw";
import codeEditorTypescriptSource from "../examples/code-editor/typescript?raw";

// markdown-editor
const MarkdownEditorDefault = lazy(() => import("../examples/markdown-editor/default"));
const MarkdownEditorWithPreview = lazy(() => import("../examples/markdown-editor/with-preview"));
const MarkdownEditorWithoutToolbar = lazy(
	() => import("../examples/markdown-editor/without-toolbar"),
);
const MarkdownEditorCustomHeight = lazy(() => import("../examples/markdown-editor/custom-height"));
const MarkdownEditorControlled = lazy(() => import("../examples/markdown-editor/controlled"));

import markdownEditorControlledSource from "../examples/markdown-editor/controlled?raw";
import markdownEditorCustomHeightSource from "../examples/markdown-editor/custom-height?raw";
import markdownEditorDefaultSource from "../examples/markdown-editor/default?raw";
import markdownEditorWithPreviewSource from "../examples/markdown-editor/with-preview?raw";
import markdownEditorWithoutToolbarSource from "../examples/markdown-editor/without-toolbar?raw";

// rich-text-editor
const RichTextEditorDefault = lazy(() => import("../examples/rich-text-editor/default"));
const RichTextEditorControlled = lazy(
	() => import("../examples/rich-text-editor/with-controlled-value"),
);
const RichTextEditorCustomToolbar = lazy(
	() => import("../examples/rich-text-editor/custom-toolbar"),
);
const RichTextEditorReadonly = lazy(() => import("../examples/rich-text-editor/readonly"));
const RichTextEditorOutputJson = lazy(() => import("../examples/rich-text-editor/output-json"));

import richTextEditorCustomToolbarSource from "../examples/rich-text-editor/custom-toolbar?raw";
import richTextEditorDefaultSource from "../examples/rich-text-editor/default?raw";
import richTextEditorOutputJsonSource from "../examples/rich-text-editor/output-json?raw";
import richTextEditorReadonlySource from "../examples/rich-text-editor/readonly?raw";
import richTextEditorControlledSource from "../examples/rich-text-editor/with-controlled-value?raw";
...
export const COMPONENT_CONTENT: Record<string, ComponentContent> = {
	...
	"code-editor": {
		...
		overview:
			"Source-code editor backed by CodeMirror 6. Pick a language with the `language` prop — the wrapper loads the matching highlighter and applies canvas-themed colours via the shared `useCodemirrorTheme()` hook. Toggle the page theme and the editor reskins automatically.",
		a11y: [
			'Pass an explicit `ariaLabel` describing the editor\'s purpose (e.g. "API config JSON").',
			"Editor content is rendered to a CodeMirror canvas; assistive tech reads through CM6's `aria-multiline=true` overlay.",
		],
		tokens: ["--foreground", "--muted-foreground", "--border", "--brand", "--muted"],
		examples: [
			{
				id: "default",
				title: "Default",
			...
			},
			{
				id: "javascript",
				title: "JavaScript",
				render: () => <CodeEditorJavascript />,
				source: codeEditorJavascriptSource,
				filename: "CodeEditor.tsx",
			},
			{
				id: "typescript",
				title: "TypeScript",
				render: () => <CodeEditorTypescript />,
				source: codeEditorTypescriptSource,
				filename: "CodeEditor.tsx",
			},
			{
				id: "json",
				title: "JSON",
				render: () => <CodeEditorJson />,
				source: codeEditorJsonSource,
				filename: "CodeEditor.tsx",
			},
			{
				id: "html",
				title: "HTML",
				render: () => <CodeEditorHtml />,
				source: codeEditorHtmlSource,
				filename: "CodeEditor.tsx",
			},
			{
				id: "css",
				title: "CSS",
				render: () => <CodeEditorCss />,
				source: codeEditorCssSource,
				filename: "CodeEditor.tsx",
			},
			{
				id: "no-line-numbers",
				title: "No gutter",
				render: () => <CodeEditorNoLineNumbers />,
				source: codeEditorNoLineNumbersSource,
				filename: "CodeEditor.tsx",
			},
		],
	},
	"markdown-editor": {
		...
		overview:
			"Markdown editor backed by CodeMirror 6 with a canvas-styled toolbar (bold / italic / lists / link / code / quote / preview-toggle). Optional split-pane preview renders sanitized markdown via marked + DOMPurify.",
		a11y: [
			'Pass an explicit `ariaLabel`. The toolbar is `role="toolbar"`; every button has an `aria-label`.',
			"Preview pane HTML is sanitized — `<script>`, inline `on*=` handlers, and `javascript:` URLs are stripped.",
		],
		tokens: ["--foreground", "--muted-foreground", "--border", "--brand", "--muted", "--card"],
		examples: [
			{
				id: "default",
				title: "Default",
			...
			},
			{
				id: "with-preview",
				title: "With preview",
				description: "Split-pane sanitized markdown preview.",
				render: () => <MarkdownEditorWithPreview />,
				source: markdownEditorWithPreviewSource,
				filename: "MarkdownEditor.tsx",
			},
			{
				id: "without-toolbar",
				title: "Without toolbar",
				render: () => <MarkdownEditorWithoutToolbar />,
				source: markdownEditorWithoutToolbarSource,
				filename: "MarkdownEditor.tsx",
			},
			{
				id: "custom-height",
				title: "Custom height",
				render: () => <MarkdownEditorCustomHeight />,
				source: markdownEditorCustomHeightSource,
				filename: "MarkdownEditor.tsx",
			},
			{
				id: "controlled",
				title: "Controlled",
				description: "Word + character count driven from the controlled value.",
				render: () => <MarkdownEditorControlled />,
				source: markdownEditorControlledSource,
				filename: "MarkdownEditor.tsx",
			},
		],
	},
	"rich-text-editor": {
		...
		overview:
			"WYSIWYG editor backed by Tiptap (ProseMirror). StarterKit ships bold / italic / strike / code / lists / blockquote / code-block / heading levels; Link adds inline links. Toolbar buttons reflect editor state via canvas's `Toggle` atom — pressed-state already token-themed bright `--brand`.",
		a11y: [
			'Pass an explicit `ariaLabel`. The editable surface is `role="textbox"` `aria-multiline`.',
			"Toolbar uses canvas atoms: every Toggle has an `aria-label`; pressed state matches the rest of canvas.",
		],
		tokens: ["--foreground", "--muted-foreground", "--border", "--brand", "--muted", "--card"],
		examples: [
			{
			...
			},
			{
				...
				description: "External buttons swap the editor's content programmatically.",
				render: () => <RichTextEditorControlled />,
				source: richTextEditorControlledSource,
				filename: "RichTextEditor.tsx",
			},
			{
				id: "custom-toolbar",
				title: "Custom toolbar",
				description: "Whitelist toolbar items via `toolbarItems`.",
				render: () => <RichTextEditorCustomToolbar />,
				source: richTextEditorCustomToolbarSource,
				filename: "RichTextEditor.tsx",
			},
			{
				id: "readonly",
				title: "Read-only",
				render: () => <RichTextEditorReadonly />,
				source: richTextEditorReadonlySource,
				filename: "RichTextEditor.tsx",
			},
			{
				id: "output-json",
				title: "JSON output",
				description: '`outputFormat="json"` returns ProseMirror JSON instead of HTML.',
				render: () => <RichTextEditorOutputJson />,
				source: richTextEditorOutputJsonSource,
				filename: "RichTextEditor.tsx",
			},
		],
	},
