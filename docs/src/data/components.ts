...
export const COMPONENTS: ComponentDoc[] = [
...

  {
    ...
    playground: {
      controls: [
        { type: "pills", key: "icon", label: "Icon", options: ["home", "user", "users", "settings", "search", "bell", "calendar", "mail", "check", "x", "plus", "trash", "download", "lock", "star", "shield"], cols: 4 },
      ...
      defaults: { icon: "home", size: 24, color: "foreground" },
      render: (s) => {
        const sz = s.size as number;
        ...
        const path = ICONS[s.icon as string] || ICONS.home;
        return `<svg width="${sz}" height="${sz}" viewBox="0 0 24 24" fill="none" stroke="${col}" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;
      },
    },
    sections: [],
    donts: [{
      dont: {
        html: `<div style="display:flex;gap:1.25rem;align-items:center">
