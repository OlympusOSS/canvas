...
// React Native codebase). The PUBLISHED @bnannier/canvas is a compiled dist
// package, but the docs develop against the LIVE SOURCE via a symlink (see
// package.json postinstall):
//   - the resolver pins the bare "@bnannier/canvas" import to src/index.ts, so
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  if (moduleName === "@bnannier/canvas") {
    // Resolve through the node_modules symlink path (docs/node_modules/@bnannier/
    ...
    return {
      type: "sourceFile",
      filePath: path.join(projectRoot, "node_modules", "@bnannier", "canvas", "src", "index.ts"),
    };
  }
