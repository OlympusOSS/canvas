...
- be027fb: Rename the Box primitive to View
  ...
  ```diff
  - import { Box } from "@bnannier/canvas";
  - <Box className="...">...</Box>
...
- 597a61f: Reorganize the component source into an atomic-design folder structure and ship
  ...
  `## Do & Don't` examples, every code block a real `@bnannier/canvas` component.
...
- fea3440: Reorganize component CSS by atomic-design level. The files that lived under
  ...
  `@bnannier/canvas/styles/components/button.css` becomes
  `@bnannier/canvas/styles/atoms/button.css` (button is an Atom; card is a
  Molecule; data-table is an Organism). The all-in-one
  `@bnannier/canvas/styles/canvas.css` entry is unchanged, so consumers using it
...
- d750bdf: Add a responsive layout utility layer. Canvas now ships display, flexbox, grid,
  ...
  no build step. Import the whole layer via `@bnannier/canvas/styles/canvas.css`
  or a single concern via `@bnannier/canvas/styles/utilities/flexbox.css`.
