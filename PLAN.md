...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 9.3.1 | Choose tool (Chromatic, Percy, Playwright screenshots, or custom) | [ ] | Evaluate options |
| 9.3.2 | Create test pages for every component | [ ] | HTML files covering all variants/states |
| 9.3.3 | Capture baselines for Light, Dark, Glass, Dark+Glass | [ ] | 4 screenshots per component |
| 9.3.4 | Add to CI pipeline | [ ] | Fail on unexpected pixel diff |
...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 10.1.1 | Token reference (all custom properties with values) | [ ] | Auto-generated from CSS |
| 10.1.2 | Component catalog (class names, variants, usage) | [ ] | HTML examples for each |
| 10.1.3 | Theming guide (light/dark, glass, density) | [ ] | Setup instructions |
| 10.1.4 | Migration guide (v2 to v3) | [ ] | Class name mapping, API changes |
| 10.1.5 | Consumer integration guide (for downstream packages) | [ ] | How canvas-react etc. should import |
| 10.1.6 | Browser support matrix | [ ] | Document required CSS features |

### 10.2 Release Checklist

| # | Item | Status | Notes |
|---|------|--------|-------|
| 10.2.1 | All Phase 0-7 items marked [x] | [-] | Phase 7 has pending visual/a11y checks |
| 10.2.2 | Changeset added for v3.0.0 | [ ] | `npx changeset` with major bump |
| 10.2.3 | CHANGELOG.md generated | [ ] | Via changeset version |
| 10.2.4 | package.json exports verified for npm consumers | [ ] | Test with `npm pack` |
| 10.2.5 | TypeScript declarations build correctly | [ ] | `tsc` produces dist/*.d.ts |
| 10.2.6 | npm publish via CI (never local) | [ ] | Push changeset, let release.yml handle it |
| 10.2.7 | Git tag created (v3.0.0) | [ ] | Automated by changesets |
| 10.2.8 | Downstream packages notified of breaking change | [ ] | canvas-react, canvas-vue, canvas-flux, canvas-react-native |
...
| `styles/components/card.css` | 36 | canvas.components |
| `styles/components/badge.css` | 83 | canvas.components |
| `styles/components/separator.css` | 14 | canvas.components |
...
| `styles/components/stat-card.css` | 77 | canvas.components |
| `styles/components/section-card.css` | 55 | canvas.components |
| `styles/components/data-table.css` | 111 | canvas.components |
...
### TypeScript (4 files, 70 lines)

| File | Lines | Exports |
|------|-------|---------|
| `src/index.ts` | 14 | Barrel re-exports |
| `src/theme.ts` | 42 | Theme, Surface, Density types + get/set/toggle functions |
...
| `.size-limit.json` | **STALE: references v2 React components; needs cleanup** |
...
| `.github/workflows/release.yml` | Automated npm publish via changesets |
