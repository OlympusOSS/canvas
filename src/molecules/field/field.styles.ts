...
//     already ships.
//   iOS (HIG / SwiftUI LabeledContent rows): the same structure with SF-style
//     tightened tracking on the label and value text (letterSpacing -0.08, the
//     same touch the iOS Badge uses) and a hair more row breathing room (14px
//     row gap) matching iOS inset-list row density. iOS has no native read-only
//     field control, so only HIG type/density conventions are applied, nothing
//     invented.
//   Android (Material 3 list/text-field rows): M3 type tracking (+0.25 on the
//     value's body-large, +0.5 on the muted label, the same tracking the
//     Android Badge/AlertDialog use), a touch more row spacing (16px) for M3
//     list-row density, and the more-rounded M3 shape on the overflow chip.
...
// Same structure; SF-style tightened tracking on the label and value text
// (letterSpacing -0.08, matching the iOS Badge) and a hair more row breathing
// room (14px row gap) for iOS inset-list row density. Keeps the same 9999
// overflow-chip radius (iOS avatars stay fully round).
export const iosSkin: FieldSkin = {
  ...
  controlStack: { flexDirection: "column", gap: 6 },
  label: (t) => ({ fontSize: 14, lineHeight: 20, fontWeight: "500", letterSpacing: -0.08, color: t.foreground }),
  message: (t, error) => ({ fontSize: 12, lineHeight: 16, color: error ? t.destructive : t["muted-foreground"] }),
  monoStyle: MONO,
};

// ---------- Android (Material 3 list / text-field rows) ----------
// Same structure; M3 type tracking (+0.25 on the value's body-large, +0.5 on the
// muted label, matching the Android Badge/AlertDialog), a touch more row spacing
// (16px) for M3 list-row density, and the more-rounded M3 shape (radius 8) on
// the overflow chip.
export const androidSkin: FieldSkin = {
  ...
  controlStack: { flexDirection: "column", gap: 6 },
  label: (t) => ({ fontSize: 14, lineHeight: 20, fontWeight: "500", letterSpacing: 0.25, color: t.foreground }),
  message: (t, error) => ({ fontSize: 12, lineHeight: 16, color: error ? t.destructive : t["muted-foreground"] }),
  monoStyle: MONO,
};
