...
// per OS:
//   iOS (HIG popover): a rounded card (~13 radius) over the `popover` material,
//     NO visible border, a soft lg shadow, ~14pt padding, with a small ARROW
//     pointing toward the anchor (up when the card is below the trigger, down
//     when above). The selection accent / action button stay the brand indigo.
...
// ---------- iOS (HIG popover): rounded material card, no border, arrow ----------
// Apple's popover: a rounded rect (~13pt) over the `popover` material with NO
// visible border, a soft drop shadow, ~14pt padding, and a small arrow pointing
// toward the anchor view. Brand type/accents survive.
const IOS_ARROW = 8; // half-width of the 16x16 rotated square that forms the pointer
export const iosSkin: PopoverSkin = {
  ...
  title: (t) => ({ ...TITLE_TYPE, color: t["popover-foreground"] }),
  description: (t) => ({ ...DESC_TYPE, color: t["muted-foreground"] }),
  // A 16x16 square rotated 45° reads as a diamond; clipped by the card edge it
  // shows as a triangular pointer. Positioned by the shell flush to the card's
  // anchor-facing edge, offset in from the left so it sits under the trigger.
  arrow: (t, placement) => ({
    position: "absolute",
    left: 20,
    width: IOS_ARROW * 2,
