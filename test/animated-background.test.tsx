...
describe("AnimatedBackground", () => {
  ...
  it("applies size, opacity, blur, and radial-gradient to each orb", () => {
    const { container } = render(
      <AnimatedBackground
        orbs={[{ color: "#abcdef", size: 300, opacity: 0.5, blur: 50 }]}
      />,
    );
    ...
    expect(orb.style.background.toLowerCase()).toMatch(
  ...
  it("falls back to defaults when size/opacity/blur are omitted", () => {
    const { container } = render(
      <AnimatedBackground orbs={[{ color: "#123456" }]} />,
    );
    const orb = (container.firstChild as HTMLElement).firstChild as HTMLElement;
  ...
  it("matches snapshot", () => {
    const { container } = render(
      <AnimatedBackground
        orbs={[
          { color: "#123456", size: 200, opacity: 0.3, blur: 40, position: "top-0 left-0" },
        ]}
      />,
    );
