...
describe("DataTable", () => {
	...
	it("renders LoadingState when loading=true (hides data)", () => {
		const columns: DataTableColumn<Row>[] = [{ field: "name", headerName: "Name" }];
	...
	it("shows emptyMessage when data is empty", () => {
		const columns: DataTableColumn<Row>[] = [{ field: "name", headerName: "Name" }];
		render(
			<DataTable columns={columns} data={[]} emptyMessage="Nothing here" pagination={false} />,
		);
	...
	it("fires onRowClick when a row is clicked", () => {
		...
		const columns: DataTableColumn<Row>[] = [{ field: "name", headerName: "Name" }];
		render(<DataTable columns={columns} data={rows} onRowClick={onRowClick} pagination={false} />);
	...
	it("matches snapshot with 3 rows + 2 columns", () => {
		...
		const { container } = render(<DataTable columns={columns} data={rows} pagination={false} />);
