...
describe("ActionBar", () => {
	it("renders the primary action label", () => {
		render(<ActionBar primaryAction={{ label: "Save", onClick: () => {} }} />);
	...
	it("calls the primary onClick handler", () => {
		const onClick = vi.fn();
		render(<ActionBar primaryAction={{ label: "Go", onClick }} />);
	...
	it("calls a secondary onClick handler", () => {
		...
		render(<ActionBar secondaryActions={[{ label: "Cancel", onClick: onCancel }]} />);
		fireEvent.click(screen.getByRole("button", { name: /Cancel/ }));
		expect(onCancel).toHaveBeenCalledTimes(1);
	});

	it("disables the primary button when loading=true", () => {
		render(<ActionBar primaryAction={{ label: "Saving", onClick: () => {}, loading: true }} />);
		const btn = screen.getByRole("button", { name: /Saving/ });
		expect(btn).toBeDisabled();
	});

	it("disables the primary button when disabled=true", () => {
		render(<ActionBar primaryAction={{ label: "Blocked", onClick: () => {}, disabled: true }} />);
		expect(screen.getByRole("button", { name: /Blocked/ })).toBeDisabled();
	});

	it("disables a secondary button when loading=true", () => {
		render(
			<ActionBar secondaryActions={[{ label: "Processing", onClick: () => {}, loading: true }]} />,
		);
	...
	it("applies align=space-between class", () => {
		const { container } = render(
			<ActionBar align="space-between" primaryAction={{ label: "Ok", onClick: () => {} }} />,
		);
